 ...
 */
class StickyHeadersGridLayoutManager<T> : GridLayoutManager where T : RecyclerView.Adapter<*>?, T : StickyHeaders? {
    ...
    private fun scrollToPositionWithOffset(position: Int, offset: Int, adjustForStickyHeader: Boolean) {
        ...
        if (mStickyHeader != null && headerIndex == findHeaderIndex(mStickyHeaderPosition)) {
            val adjustedOffset = (if (offset != LinearLayoutManager.INVALID_OFFSET) offset else 0) + mStickyHeader!!.height
    ...
    override fun onFocusSearchFailed(focused: View, focusDirection: Int, recycler: Recycler,
